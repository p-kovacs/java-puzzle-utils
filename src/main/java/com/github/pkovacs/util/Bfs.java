 ...
 * infinite graphs provided that the adjacent nodes are generated on-the-fly when requested by the algorithm.
 * For example, the nodes and edges might represent the feasible states and steps of a game or a combinatorial
 * problem, respectively, and we might not be able to or do not want to enumerate all possible (and reachable)
 * states in advance.
 *
 * @see Dijkstra
 * @see BellmanFord
...
public final class Bfs {
    ...
    /**
     * Calculates the distance (number of edges) along one of the shortest paths from the given source node to the
     * nearest target node identified by the given predicate.
     *
     * @param graph the {@link Graph} that provides the adjacent nodes for each node.
